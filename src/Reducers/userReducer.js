...
const userReducer = (state=initialState, action) => {
    ...
    switch(action.type){
        ...
        // set the user cart. spread the cart and add the new item from the action to it.
        case "ADD_TO_CART":
        ...
        case "REMOVE_FROM_CART":
            ...
            // reset the state by spreading the state and adding the new cart without the deleted item.
            return {...state, cart: newArr}

        // creates an order. When the order gets created the cart gets cleared out cart: [] will clear cart with an empty array. The order is then added to the orders array by spreading the past orders and adding the new one from the action.
        case "CREATE_ORDER":
